...
Page({
  ...
  async loadPageData(isRefresh = false) {
    ...
    try {
      ...
      this.setData({
        ...
        submitButtonText: fallbackText(siteConfig.contactPageSubmitText, "提交留言"),
        submitSuccessText: fallbackText(siteConfig.contactSubmitSuccessText, "留言已提交，我们会尽快联系你"),
        consultButtonText: fallbackText(siteConfig.consultButtonText, "联系门店"),
  ...
  normalizeFormData(formData: ContactForm) {
    return {
  ...
  validateFormData(formData: ContactForm) {
    if (!formData.name) {
      return "请填写你的称呼";
    }
    if (!formData.phone) {
