...
## 5. 为什么后台里会看到更多 Tabs 和工作区

因为最近后台已经把几个长页面统一收成了更稳定的工作区结构：

- `站点配置`：通过 Tabs 管理首页、联系、品牌故事、关于我们、后台文案、媒体资源
- `运维中心`：通过 Tabs 管理总览、备份、巡检、安全、归档、迁移
- `操作日志`：独立为筛选、恢复、快照对比工作台
...
- 二开：看 [系统架构](./architecture.md) 和 [本地开发环境说明](./local-development-guide.md)
