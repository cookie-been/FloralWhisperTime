    ...
    <Link to={`/gallery/${flower.id}`} className="surface-card group block overflow-hidden transition hover:-translate-y-1">
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h3 className="text-base font-semibold text-ink">{flower.name}</h3>
            <p className="mt-1 line-clamp-2 text-sm leading-6 text-muted">{flower.description}</p>
          </div>
          <span className="shrink-0 text-sm font-semibold text-forest">¥{flower.price}</span>
        </div>
        {!compact && (
