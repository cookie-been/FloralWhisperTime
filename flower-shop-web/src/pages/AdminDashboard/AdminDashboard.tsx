...
import { buildDashboardAuditCards, buildDashboardStats, buildDashboardSummary, type DashboardData } from "./admin-dashboard.helpers";
...
export function AdminDashboard() {
  ...
  const stats = buildDashboardStats(data, summary).map((item, index) => ({
    ...item,
    icon: [Flower2, Star, Sparkles, ImageIcon][index],
  }));

  const auditCards = buildDashboardAuditCards(data, summary).map((item, index) => ({
    ...item,
    icon: [AlertTriangle, RotateCcw][index],
  }));
      ...
      <section className="grid gap-6 xl:grid-cols-[0.88fr_1.12fr]">
        <div className="admin-panel admin-shell-card sm:p-6">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <div className="surface-card admin-shell-subcard px-4 py-4">
            ...
            <Link to="/admin/flowers?featured=normal" className="surface-card admin-shell-subcard block px-4 py-4">
          ...
          <div className="mt-5 admin-subpanel px-4 py-4">
            ...
            <p className="mt-3 text-sm leading-7 text-muted">
              {summary.latestFailedLog
                ? `最近失败项来自${formatAdminModuleLabel(summary.latestFailedLog.module)}，发生于 ${formatDate(summary.latestFailedLog.createdAt)}，建议优先核对失败原因并确认是否需要回滚。`
          ...
          <div className="mt-5 space-y-3">
            {data.failedOperationLogs.list.length ? (
              data.failedOperationLogs.list.map((item) => (
                <Link key={item.id} to="/admin/operation-logs?success=false" className="surface-card admin-shell-subcard block px-4 py-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-sm font-semibold text-[#1b281e]">
                        {formatAdminModuleLabel(item.module)} · {formatAdminActionLabel(item.action)}
                      </p>
                      <p className="mt-1 text-sm text-muted">
                        {formatAdminTargetTypeLabel(item.targetType)} / {formatDate(item.createdAt)}
                      </p>
                      <p className="mt-2 line-clamp-2 text-sm leading-6 text-[#9f4b45]">{item.errorMessage || item.requestSummary || "失败日志"}</p>
                    </div>
                    {renderAdminSuccessTag(false)}
                  </div>
                </Link>
              ))
            ) : (
            ...
            <div>
              ...
              <div className="mt-3 space-y-3">
                {summary.recentFlowers.map((flower) => (
                  <div key={flower.id} className="surface-card admin-shell-subcard px-4 py-3">
